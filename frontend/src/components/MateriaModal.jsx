...
export function MateriaModal({ open, onClose, initial = null, onSave }) {
  ...
  useEffect(() => {
    if (initial) {
      ...
      setNombre(''); setDescripcion(''); setCreditos(2); setActivo(true);
    }
  }, [initial, open]);

  useEffect(() => {
    const handler = (e) => { if (e.key === 'Escape' && open) onClose && onClose(); };
  ...
  const submit = (e) => {
    e.preventDefault();
    const payload = { ...initial, titulo: nombre.trim() || 'Materia', descripcion, numero_creditos: Number(creditos), activo };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold">{initial ? 'Editar Materia' : 'Nueva Materia'}</h3>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div>
            ...
            <textarea value={descripcion} onChange={(e) => setDescripcion(e.target.value)} rows={3} className="w-full p-2 border rounded" />
          </div>
