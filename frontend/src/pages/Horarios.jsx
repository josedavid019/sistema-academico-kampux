  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const materias = Array.isArray(materiasList)
          ? materiasList
          : materiasList.results ?? materiasList;
        const aulas = Array.isArray(aulasList)
          ? aulasList
          : aulasList.results ?? aulasList;
        ...
        const mapped = (horarios || []).map((h) => {
          ...
          const fin = new Date(
            inicio.getTime() + DEFAULT_DURATION_MINUTES * 60 * 1000
          );

          return {
            id: h.id,
            materia:
              materiaObj.nombre_materia ||
              materiaObj.nombre ||
              `Materia ${h.materia}`,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4 gap-2">
          <button
            ...
            className="px-3 py-2 rounded-lg bg-blue-100 text-blue-700 font-bold hover:bg-blue-200"
          >
            ←
          </button>
          <div className="font-semibold text-lg text-gray-800">
            ...
            -{" "}
            {semana[6].toLocaleDateString("es-ES", {
              day: "numeric",
              month: "short",
              year: "numeric",
            })}
          ...
          <button
            ...
            className="ml-4 px-4 py-2 rounded-lg bg-green-600 text-white font-bold hover:bg-green-700 shadow"
          >
            Imprimir horario
          </button>
        </div>
          ...
          <div className="grid grid-cols-8 border rounded-lg bg-white shadow">
            <div className="col-span-1 border-r bg-blue-50">
              {horas.map((h) => (
                <div
                  key={h}
                  className="h-12 flex items-center justify-center text-xs text-gray-500 border-b"
                >
                  {h}
                </div>
              ))}
                ...
                <div className="relative">
                  {clasesSemana
                    ...
                    .map((clase, i) => {
                      ...
                      const top =
                        (inicio.getHours() - 8) * 48 +
                        (inicio.getMinutes() / 60) * 48;
                      ...
                      return (
                        ...
                        >
                          ...
                          {clase.aula && (
                            <span className="text-gray-300">{clase.aula}</span>
                          )}
                        </div>
                      );
                    })}
            ...
            {!loading && resultItems.length > 0 && (
              <div>
                <div className="space-y-2">
                  {resultItems.map((r) => {
                    const materiaName =
                      r.materia_name || r.materia || `Materia ${r.materia}`;
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      disabled={resultItems.length < resultPageSize}
