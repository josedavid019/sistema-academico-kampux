...
export function AdminUsuarios() {
  ...
  const loadAcademico = async () => {
    try {
      ...
      console.error("Error cargando facultades/programas", err);
    }
  };
  ...
  const loadLists = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "No se pudieron cargar usuarios para mapear perfiles",
          errUsers
        );
      }
    } catch (err) {
      ...
      setListsError(
        err.response?.data || err.message || "Error cargando listas"
      );
    } finally {
  ...
  const handleEditUsuario = async (userId) => {
    ...
    if (!usuario) {
      toastError("Información del usuario no disponible para editar.");
      return;
    }
  ...
  const filteredEstudiantes = estudiantes.filter((e) => {
    ...
    return (
      name.includes(q) ||
      email.includes(q) ||
      (e.programa?.nombre_programa || "").toLowerCase().includes(q)
    );
  });
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <SectionHeader
          title="Usuarios"
          subtitle="Crear perfiles de usuarios (estudiantes, docentes, administradores)"
        />
        <div className="bg-white rounded-lg shadow-md p-6 mt-6">
          <p className="text-sm text-gray-600">
            Gestión centralizada de usuarios: crea y administra perfiles de
            Estudiante, Docente y Administrador desde aquí.
          </p>
          <div className="mt-6">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  <input
                    ...
                    className="w-72 pl-10 pr-3 py-2 border border-gray-200 rounded-md shadow-sm text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500"
                  />
                  <svg
                ...
                <button
                  onClick={loadLists}
                  className="px-3 py-2 bg-gray-100 text-gray-700 rounded-md text-sm border border-gray-200"
                >
                  Refrescar
                </button>
              </div>
              <div className="flex items-center gap-2">
                {!showStudentModal && (
                  <button
                    ...
                    className="px-4 py-2 bg-indigo-700 text-white rounded-md text-sm shadow"
                  >
                    Crear Estudiante
                  </button>
                )}
                <button
                  ...
                  className="px-4 py-2 bg-green-700 text-white rounded-md text-sm shadow"
                >
                  Crear Docente
                </button>
                <button
                  onClick={() => setShowAdminModal(true)}
                  className="px-4 py-2 bg-yellow-600 text-white rounded-md text-sm shadow"
                >
                  Crear Administrador
                </button>
              </div>
            ...
            {loadingLists && (
              <div className="text-sm text-gray-600">Cargando listas...</div>
            )}
            {listsError && (
                          ...
                          {filteredDocentes.map((d) => (
                            <tr key={d.id} className="hover:bg-gray-50">
                              <td className="px-3 py-2">
                                {`${usersMap[d.user]?.nombre || ""} ${
                                  usersMap[d.user]?.apellido || ""
                                }`.trim()}
                              </td>
                              <td className="px-3 py-2">
                                {usersMap[d.user]?.email || ""}
                              </td>
                              <td className="px-3 py-2">
                                {usersMap[d.user] && (
                                  <div className="flex items-center gap-2">
                                    <button
                                      onClick={() => handleEditUsuario(d.user)}
                        ...
                        <tbody className="bg-white divide-y divide-gray-100">
                          {filteredEstudiantes.map((e) => (
                            <tr key={e.id} className="hover:bg-gray-50">
                              <td className="px-3 py-2">
                                {`${usersMap[e.user]?.nombre || ""} ${
                                  usersMap[e.user]?.apellido || ""
                                }`.trim()}
                              </td>
                              <td className="px-3 py-2">
                                {usersMap[e.user]?.email || ""}
                              </td>
                              <td className="px-3 py-2">
                                {e.programa?.nombre_programa || ""}
                              </td>
                              <td className="px-3 py-2">
                                {usersMap[e.user] && (
                                  <div className="flex items-center gap-2">
                                    <button
                                      onClick={() => handleEditUsuario(e.user)}
                                      className="px-2 py-1 border border-indigo-200 text-indigo-700 rounded text-xs hover:bg-indigo-50"
                                    >
                                      Editar
                                    </button>
                        ...
                        <tbody className="bg-white divide-y divide-gray-100">
                          {filteredAdministradores.map((a) => (
                            <tr key={a.id} className="hover:bg-gray-50">
                              <td className="px-3 py-2">
                                {`${usersMap[a.user]?.nombre || ""} ${
                                  usersMap[a.user]?.apellido || ""
                                }`.trim()}
                              </td>
                              <td className="px-3 py-2">
                                {usersMap[a.user]?.email || ""}
                              </td>
                              <td className="px-3 py-2">
                                {usersMap[a.user] && (
                                  <div className="flex items-center gap-2">
                                    <button
                                      onClick={() => handleEditUsuario(a.user)}
                                      className="px-2 py-1 border border-indigo-200 text-indigo-700 rounded text-xs hover:bg-indigo-50"
                                    >
                                      Editar
                                    </button>
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center">
                ...
                <div className="bg-white rounded-lg shadow-lg z-10 w-full max-w-md p-6">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    ...
                    >
                      <option value="">
                        -- Selecciona Programa (opcional) --
                      </option>
                      {programas
                        .filter(
                          (p) =>
                            !studentForm.facultad ||
                            String(p.facultad) === String(studentForm.facultad)
                        )
                        .map((p) => (
                    ...
                    <div className="flex justify-end gap-2 mt-2">
                      ...
                      <button
                        onClick={async () => {
                          ...
                          if (!email) {
                            toastError("Email requerido");
                            return;
                          }
                          const password = randomPassword(10);
                          try {
                            const user = await registerUsuario({
                              email,
                              password,
                              password2: password,
                              nombre,
                            ...
                            toastSuccess(
                              "Estudiante creado. Contraseña temporal: " +
                                password
                            );
                            setStudentForm({
                              email: "",
                ...
                <div className="bg-white rounded-lg shadow-lg z-10 w-full max-w-md p-6">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    <input
                      ...
                      onChange={(e) =>
                        setDocenteForm((s) => ({ ...s, email: e.target.value }))
                      }
                      className="p-2 border rounded"
                    />
                    <input
                          ...
                          try {
                            ...
                            const msg = err.response?.data
                              ? typeof err.response.data === "string"
                                ...
                                : JSON.stringify(err.response.data)
                              : err.message || String(err);
                            toastError("Error creando docente: " + msg);
                          }
                        }}
                ...
                <div className="bg-white rounded-lg shadow-lg z-10 w-full max-w-md p-6">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAdminForm((s) => ({ ...s, nombre: e.target.value }))
                      }
                      className="p-2 border rounded"
                    />
                    <input
                          ...
                          try {
                            ...
                            const msg = err.response?.data
                              ? typeof err.response.data === "string"
                                ...
                                : JSON.stringify(err.response.data)
                              : err.message || String(err);
                            toastError("Error creando administrador: " + msg);
                          }
                        }}
                ...
                <div className="bg-white rounded-lg shadow-lg z-10 w-full max-w-md p-6">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditForm((s) => ({ ...s, apellido: e.target.value }))
                      }
                      className="p-2 border rounded"
                    />
                    <div className="flex justify-end gap-2 mt-2">
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            await putUsuario(editingUserId, {
                              email,
                              nombre,
                              apellido,
                            });
                            ...
                            const msg = err.response?.data
                              ? typeof err.response.data === "string"
                                ...
                                : JSON.stringify(err.response.data)
                              : err.message || String(err);
                            toastError("Error actualizando usuario: " + msg);
                          }
                        }}
                        className="px-4 py-2 bg-indigo-600 text-white rounded"
                      >
