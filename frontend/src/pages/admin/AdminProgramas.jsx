...
import { getProgramas, postPrograma, putPrograma, deletePrograma } from '../../api/academico.api';
...
export function AdminProgramas() {
  ...
  const handleEdit = (p) => { setEditing({ id: p.id, titulo: p.nombre_programa, descripcion: p.descripcion }); setModalOpen(true); };
  ...
  const handleSave = async (payload) => {
    try {
      if (payload.id) {
        await putPrograma(payload.id, { nombre_programa: payload.titulo, descripcion: payload.descripcion });
      } else {
        await postPrograma({ nombre_programa: payload.titulo, descripcion: payload.descripcion });
      }
      await load();
    } catch (err) { console.error(err); alert('Error guardando programa'); }
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-6">
          <div className="flex justify-end mb-4">
            <button onClick={handleCreate} className="px-4 py-2 bg-blue-900 text-white rounded">+ Nuevo programa</button>
          </div>
        ...
        <ProgramaModal open={modalOpen} onClose={() => setModalOpen(false)} initial={editing} onSave={handleSave} />
      </div>
    </div>
  );
}
