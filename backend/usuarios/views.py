...
class UsuarioDetailAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    ]  # requiere estar autenticado para modificar/eliminar
...
class EstudianteCreateAPIView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            obj = serializer.save()
            return Response(
                EstudianteCreateSerializer(obj).data, status=status.HTTP_201_CREATED
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class DocenteCreateAPIView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            obj = serializer.save()
            return Response(
                DocenteCreateSerializer(obj).data, status=status.HTTP_201_CREATED
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class CoordinadorCreateAPIView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            obj = serializer.save()
            return Response(
                CoordinadorCreateSerializer(obj).data, status=status.HTTP_201_CREATED
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class AdministradorCreateAPIView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            obj = serializer.save()
            return Response(
                AdministradorCreateSerializer(obj).data, status=status.HTTP_201_CREATED
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class EstudianteListAPIView(generics.ListAPIView):
    queryset = (
        Estudiante.objects.select_related("user", "facultad", "programa")
        .all()
        .order_by("id")
    )
...
class CoordinadorListAPIView(generics.ListAPIView):
    queryset = (
        Coordinador.objects.select_related("user", "programa").all().order_by("id")
    )
