...
class HorarioEstudianteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        """
        estudiante = data.get("estudiante") or getattr(
            self.instance, "estudiante", None
        )
        ...
        if qs.exists():
            raise serializers.ValidationError(
                "El estudiante ya tiene un horario para ese día/hora/grupo."
            )
...
class HorarioDocenteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if docente:
            ...
            if qs_doc.exists():
                raise serializers.ValidationError(
                    "El docente ya tiene una clase en ese día/hora/grupo."
                )
        ...
        if qs_aula.exists():
            raise serializers.ValidationError(
                "El aula ya está asignada a otra clase en ese día/hora."
            )
